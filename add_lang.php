...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if($name) {
            $stmt = $pdo->prepare("INSERT INTO languages (name, country, speakers, status, ai_tool, region, description) VALUES (?, ?, ?, ?, ?, ?, ?)");
            $stmt->execute([$name, $country, $speakers, $status, $ai_tool, $region, $description]);
            header('Location: admin.php?msg=New+Language+Added#languages');
            exit;
        } else {
...
<head>
    ...
    <title>Add Language Entry - Admin</title>
    ...
    <form method="POST">
        <div class="grid-2">
            <div class="fg">
                ...
                <input type="text" name="name" placeholder="e.g. Ainu" required>
            </div>
            <div class="fg">
                <label>Status</label>
                <select name="status">
        ...
        <div class="grid-2">
            <div class="fg">
                <label>Country</label>
                <input type="text" name="country" placeholder="e.g. Japan">
            </div>
            <div class="fg">
        ...
        <div class="grid-2">
            <div class="fg">
                ...
                <input type="text" name="speakers" placeholder="e.g. ~10">
            </div>
            <div class="fg">
                <label>AI Tool Support</label>
                <input type="text" name="ai_tool" placeholder="e.g. Speech Archive">
            </div>
        </div>
        
        <div class="fg">
            <label>Description</label>
            <textarea name="description" placeholder="A brief background on the language..."></textarea>
        </div>

        <div style="margin-top: 25px;">
            <button type="submit" class="btn-submit">Add to Atlas</button>
        </div>
    </form>
</div>
